...
Phase 7a is not considered closed until the following are observed and recorded:
...
the Worker code and Cloudflare stores must remain available until the checks are
recorded.

## Data-retention boundary

No D1-to-Postgres reconciliation, backfill, or customer-notice process is
needed. The project owner has authorized discarding pre-launch operational rows.
Before any destructive database command, verify the exact host, database name,
schema, migration version, and environment; print and review the target table
counts.

Rows may be truncated or discarded only from these operational ledgers when they
are present and are confirmed to be test/pre-launch traffic:
...
Do not delete or truncate `users`, `api_keys`, `subscriptions`, `plans`, or any
Go reference/product-data table, including `advice`, `quotes`, `words`,
`bin_data`, `inflation_data`, `iban_countries`, `commodity_price_history`,
`exercises`, `swift_codes`, `counters`, or other tables containing API data. Do
not use `CASCADE`, wildcards, schema-wide truncation, or a migration that drops
a table merely because its rows are disposable. Use an explicit five-table
...
2. Confirm the public hostname, DNS proxy status, Caddy AOP configuration, Kamal
   routing, Rails private URL, and Go health/auth behavior.
3. Run a successful Kamal-managed deploy/restart using the committed routing
   ...
   URL. Do not treat the one-off live container state from Phase 7a as the final
   deployment proof.
4. Capture the Worker route/custom-domain status and the valid-key/401/429/
   Playground smoke results in the implementation notes. If no suitable live key
   exists, create a disposable pre-launch key through the normal Rails
...
understands that re-enabling the Worker route and restoring KV/D1 will no longer
be rollback options, and the remaining rollback is limited to reverting
...
scheduled, queued, retry, and dead sets for `SyncD1UsageJob`; also confirm there
is no currently running or leased instance. Observe at least one former schedule
interval with no D1 job or D1 API log entry. Only after active, queued, retry,
and scheduled work are clear may D1 be deleted. Do not change
...
With the owner present and confirming each destructive action immediately before
it runs:

1. Identify the exact production Worker names, route/custom-domain bindings, KV
   namespace ID, and D1 database name/ID from the live account and current
...
3. Delete the production `auth-gateway` and `api-management` Workers and remove
   their live custom-domain/route bindings if any remain. Verify their deployed
   secrets are no longer active; in particular, check `BACKEND_SECRET` and
   `API_MANAGEMENT_API_KEY` rather than assuming Worker deletion cleaned every
   secret.
4. Delete the Cloudflare KV namespace and D1 database used by the Workers. The
   data is disposable under this plan, but the exact resource identity and owner
   confirmation remain mandatory because the deletion is irreversible.
5. Verify that requests to the public API still terminate at the direct
   ...
   routes, triggers, custom domains, KV/D1 identifiers and deletion output; pair
   a cache-busting public request's Cloudflare Ray ID with matching Caddy and Go
   access-log entries. Check the retired `api-management.requiems.xyz` hostname
   separately.
...
- Worker path filters, `worker-test`, `api-management-test`, and their reusable
  `.github/workflows/_worker-ci.yml` workflow.
...
- The `dashboard` and `sidekiq` dependency edges so they depend on the Go `api`
  service (the private `requiems-api:8080` destination), not the deleted
  `api-management` service.
- The MCP dev service's `REQUIEMS_BASE_URL` and dependency edge so MCP uses the
  direct Go API service (`api:8080`) or the explicitly chosen public API URL,
  not the deleted `auth-gateway` service.

Do not remove a generic `BACKEND_SECRET` or tenant/private-deployment secret
solely because its name resembles the retired Worker secret. First classify each
remaining reference. Remove the old Go/Caddy/Rails proxy secret only when the
reference search and tests show it no longer serves a supported feature;
...
- `api.requiems.xyz`, `api-management.requiems.xyz`, and every discovered Worker
  custom domain/route have an intentional post-deletion DNS/HTTP state; only
  `api.requiems.xyz` remains an active public API path.
...
This phase is the reviewable cleanup pass after Phase 8's deletion. It should be
a separate commit/PR from the irreversible Cloudflare deletion where practical,
so a documentation or test mistake cannot obscure the production cutover.
...
current architecture documentation. Review product/blog copy separately and only
change it if it falsely describes the current runtime rather than an historical
technology choice.
...
- Update `.github/workflows/codeql.yml` to retain JavaScript/TypeScript analysis
  for `apps/mcp` while removing the deleted Worker target, and remove obsolete
  Worker flags from `codecov.yml` and Worker-only ignore entries from
  `.gitignore`.
...
"false positive"; it must be removed or explicitly allowlisted as historical or
product content. The allowlist should identify any retained blog, translation,
or historical explanatory copy separately from operational docs.

Delete variables and `AppConfig` readers that have no remaining consumer. Create
a usage matrix for `BACKEND_SECRET`: remove it from the normal Rails-to-Go
Playground request once Go API-key auth is the only supported credential, while
preserving the separate private-deployment tenant-secret contract and its
tests/documentation if it is still real. Remove stale tests, locales, rake-task
output, and app-config documentation along with each deleted setting.
...
disposable Redis instance. Add a guard that aborts if the host/database does not
match the expected test target. Capture before/after counts (and, where
practical, data-only checksums) for all protected product-data tables around the
suite.
...
deletion. Verify direct-origin AOP with the actual TLS `certificate_required` or
equivalent client-certificate failure, not just a generic connection failure.
Check every prior scheduler (Solid Queue and Sidekiq) for no D1 job or D1 log
activity.

The post-deletion evidence matrix must include: Worker list/routes/triggers and
custom domains before and after; exact KV/D1 identifiers and deletion output; no
Worker invocation evidence; the retired API Management hostname's DNS and HTTP
behavior; Cloudflare Ray ID plus origin logs for a cache-busting request; and
timestamps for the valid-key, invalid-key, 429, Playground, and AOP tests.
...
Three independent reviewers audited this plan after it was drafted. Findings are
classified below and incorporated into the plan unless explicitly marked
otherwise.
...
- **Running/leased D1 jobs — fixed in 8.2.** Cover both Solid Queue and Sidekiq,
  drain/restart workers, inspect scheduled/queued/retry/dead sets, and observe a
  former schedule interval before deleting D1.
- **One-off Phase 7a deployment state — fixed in 8.1.** Require a Kamal-managed
  deploy/restart followed by AOP, routing, and Rails private-URL verification.
- **Repository reference completeness — fixed in 8.5/9.1/9.3.** Add MCP, CodeQL,
  Copilot, generated docs, integration/load setup files, Compose seed wiring,
  ports, Wrangler forms, and a residual-reference manifest.
...
- **Validation commands — fixed in 9.4.** Replace vague JS/Compose language with
  direct stack, health, Go, Rails, MCP, integration, security, and config
  validation commands.
...
- **Historical-reference handling — fixed in 9.1/9.3.** Historical plans and
  audits remain excluded, while every other retained match requires an explicit
  manifest entry.
...
Completed 2026-08-23 UTC after explicit owner approval immediately before the
irreversible Cloudflare deletion calls.

### Phase 7a verification and production bake
...
- The direct Cloudflare -> Caddy -> Kamal -> Go path was exercised with a
  ...
  the public Playground request returned 200 after the dashboard redeploy.
- Representative pre-deletion Ray/request evidence included
  `a2f682b79880b93f-AMS` / `8e488d87-65f2-4619-a6b2-3a9e850dd2d1` (valid),
  `a2f68441afdaf794-AMS` / `fbab0927-7fd5-4a2b-a908-29159f087a96` (401), and
  `a2f684d40aadfe97-AMS` / `6f49d9dc-1a6d-4f3c-85bc-e9070e2ab1a9` (429).
  Matching Go logs recorded statuses 200, 401, and 429 on the expected routes.

### Scheduler drain and deletion evidence
...
- The old Phase 7 dashboard web/job containers were drained and retired after
  the worker inspection; only the new `48f87a0d` dashboard web/job containers
  remain running.
...
- KV namespace delete: `7cc847da3f3143b2ba8f7c531f416b35`, title
  `requiems_api_cf`; Cloudflare returned `success: true`.
- D1 database delete: UUID `8cc5cae5-caff-4878-80f7-94eed1931ea8`, name
  `requiem-usage`; the delete completed and the subsequent account list no
...
- Removed the tracked `apps/workers/` application code, both Worker Dockerfiles,
  Worker Compose services/mounts/volumes, Wrangler/Worker CI and Dependabot
  wiring, API Management/D1 Rails services/jobs/tests/locales, D1 schedules,
  stale normal Rails-to-Go secrets, and obsolete Worker/API Management docs.
- Rewired dashboard, Sidekiq, MCP, integration/load setup, generated-doc
...
### Database boundary and protected data

- No PostgreSQL cleanup command was run. Exact production identity was verified
...
- Post-delete smoke at `2026-08-23T02:13Z`: health 200 Ray
  ...
  `e16340dc-25ee-482a-9d2d-5f5afb90979a`. Matching Go logs recorded all four
  statuses.
- `api-management.requiems.xyz` failed DNS resolution (`curl` exit 6). Direct
  origin TLS remained rejected (`curl` exit 56, alert 1116).

### Unresolved findings and follow-up

- Intentional residual matches are limited to unchanged historical audit/plan
  records, historical product/blog content, generic Go/web-worker terminology,
  Cloudflare Origin Pull CA material, and the private tenant-secret contract.
  They are allowlisted in the implementation review and should not be removed
  without a separate content/security decision.
- No production traffic or billing reconciliation/backfill was performed, as
  directed. Revisit usage retention, Redis rate-limit behavior, and operational
  observability only after real traffic exists; no further migration work is
  required for the retired Worker/KV/D1 path.
