...
1. Read [backend patterns](backend.md) and the service's existing
   `service.go`, `transport_http.go`, and route registration.
2. Add the handler and service under the appropriate `apps/api/services`
   domain.
3. Register the route under the Go router and keep the
   `requiems-api-key` middleware boundary intact.
...
6. Update the dashboard API documentation snippets if the public contract
   changed.
7. Run `go test ./...`, MCP tests/typecheck, Rails checks when affected, and
   the integration smoke checks.
