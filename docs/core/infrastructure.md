...
The retired `api-management.requiems.xyz` hostname has no application service
...
Public API authentication and authorization are Go middleware concerns. Caddy's
mTLS check prevents direct-origin bypass. Rails' private Playground connection
is network-private and still sends a Go API key. Private deployment customers
have a separate `tenant_secret` contract; that is not the retired normal
Rails-to-Go `BACKEND_SECRET` setting.
