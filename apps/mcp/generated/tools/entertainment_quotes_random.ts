// AUTO-GENERATED by scripts/generate.ts — do not edit by hand.
// Source operation: GET /v1/entertainment/quotes/random
// Regenerate with: bun run scripts/generate.ts --input <spec> --output generated

import { z } from "zod";
import { requiemsRequest } from "../runtime";

/**
 * Returns a random inspirational quote with author attribution
 */
export const inputSchema = z.object({});

export type EntertainmentQuotesRandomInput = z.infer<typeof inputSchema>;

export const entertainment_quotes_random = {
  name: "entertainment_quotes_random",
  description: "Returns a random inspirational quote with author attribution",
  inputSchema,

  handler: async (rawArgs: Record<string, unknown>) => {
    const args = inputSchema.parse(rawArgs);
    const result = await requiemsRequest({
      method: "GET",
      path: "/v1/entertainment/quotes/random",
      pathParams: undefined,
      query: undefined,
      body: undefined,
    });
    return result;
  },
};
