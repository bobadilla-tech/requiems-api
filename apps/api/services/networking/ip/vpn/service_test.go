...
func TestService_CheckBatch(t *testing.T) {
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			result, err := svc.CheckBatch(context.Background(), tt.ips)

			require.NoError(t, err)
			require.Len(t, result.Results, tt.expectedLen)
...
func TestService_CheckBatch_PreservesOrder(t *testing.T) {
	...
	result, err := svc.CheckBatch(context.Background(), ips)

	require.NoError(t, err)
	require.Len(t, result.Results, len(ips))

	for i := range ips {
		assert.Equal(t, ips[i], result.Results[i].IP)
	}
}
