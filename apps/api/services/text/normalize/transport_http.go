...
func RegisterRoutes(router chi.Router, svc *Service) {
	router.Post("/normalize/batch", httpx.HandleBatch(func(_ context.Context, req BatchEmailNormalizationRequest) (EmailNormalizationBatchResponse, error) {
		return svc.NormalizeBatch(req.Emails), nil
	}))
