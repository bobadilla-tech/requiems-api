...
func TestService_RandomBatch_AllValid(t *testing.T) {
	...
	require.Len(t, resp, len(categories))
...
func TestService_RandomBatch_InvalidCategory_InBand(t *testing.T) {
	...
	assert.NotEmpty(t, resp[0].Fact)
	assert.Empty(t, resp[0].Error)

	assert.Empty(t, resp[1].Fact)
	assert.NotEmpty(t, resp[1].Error) // absorbed in-band

	assert.NotEmpty(t, resp[2].Fact)
	assert.Empty(t, resp[2].Error)
}
